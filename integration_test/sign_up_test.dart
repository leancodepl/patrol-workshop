...
void main() {
  patrolTest(
    ...
    ($) async {
      // log in to Google account on device
      await googleAccountSetUp($.native);
      ...
      await $(K.signInWithGoogleButton).tap(); //
      ...
      // log out from Google account on device
      await googleAccountTearDown($.native);
    },
  );
}

Future<void> googleAccountSetUp(NativeAutomator automator) async {
...
Future<void> googleAccountTearDown(NativeAutomator automator) async {
  await automator.openApp(appId: 'com.android.settings');
  await swipeUntilVisible(
    automator: automator,
    elementSelector: Selector(
